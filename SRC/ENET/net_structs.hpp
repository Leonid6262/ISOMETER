...
static constexpr unsigned short _mac_length = 6;
static constexpr unsigned short _ip_length = 4;

// 1. Заголовок Ethernet (14 байт)
struct SEthernetHeader {
    unsigned char  dest_mac[_mac_length];
    unsigned char  src_mac[_mac_length];
...
struct SARPHeader {
    ...
    unsigned char  sender_mac[_mac_length];
    unsigned char  sender_ip[_ip_length];
    unsigned char  target_mac[_mac_length];
    unsigned char  target_ip[_ip_length];
};
...
struct SIPHeader {
    ...
    unsigned char  src_ip[_ip_length];
    unsigned char  dest_ip[_ip_length];
};
